...
document.querySelectorAll(".post .like-btn").forEach((btn) => {
  btn.addEventListener("click", () => {
    ...
    if (user === "@Sam") {
      ...
      reply.textContent = "1 new comment: 'lol'";
      ...
      sideText.innerHTML = `<p>You liked ${user}'s post. Social Media usage can reduce face-to-face interactions, making it difficult for people to fotm "real" social relationships.</p>`;
    }
  });
});
...
toSection4Btn.addEventListener("click", () => {
  ...
  setTimeout(() => {
    ...
    setTimeout(() => (section4.style.opacity = "1"), 100);
  }, 1000);
});
